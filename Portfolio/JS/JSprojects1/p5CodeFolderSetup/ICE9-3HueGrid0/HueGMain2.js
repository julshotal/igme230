...
function setup() {
    ...
    strokeColSlct.option('black', 0); //stroke is black
    ...
    for (let i = 0; i < rowLeng; i++) {
        ...
        for (let j = 0; j < colLeng; j++) {
            ...
            switch (val) { //based on the pattern chosen in the drop down menu, the grid is reset in that pattern. Hyperbolic is automaticall picked at beginning
                case 'Hyperbolic':
                    hueVal[i][j] = (i * j) % colVal; //formula for a hyperbole 
                    break;
                case 'Striped':
                    hueVal[i][j] = ((i + j) * scale) % colVal; //normalize the hue value to save graphics card stress, it would have to normalize the hue value but we can just do it here
                    break;
                case 'Random':
                    hueVal[i][j] = random(colVal);
                    break;
            }
        }
    }
